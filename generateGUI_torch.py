...
playerNetworks = {}
playerNetworks[1] = NeuRDSequentialNetwork(inDim, 1695, {"pi": [512,512], "vf": [256]}, nn.ReLU, "cpu")
playerNetworks[2] = NeuRDSequentialNetwork(inDim, 1695, {"pi": [512,512], "vf": [256]}, nn.ReLU, "cpu")
playerNetworks[3] = NeuRDSequentialNetwork(inDim, 1695, {"pi": [512,512], "vf": [256]}, nn.ReLU, "cpu")
playerNetworks[4] = NeuRDSequentialNetwork(inDim, 1695, {"pi": [512,512], "vf": [256]}, nn.ReLU, "cpu")
...
playerNetworks[2].load_state_dict(th.load(parameters_name, map_location=th.device('cpu')))
playerNetworks[3].load_state_dict(th.load(parameters_name, map_location=th.device('cpu')))
playerNetworks[4].load_state_dict(th.load(parameters_name, map_location=th.device('cpu')))

playerNetworks[1].eval()
playerNetworks[2].eval()
playerNetworks[3].eval()
playerNetworks[4].eval()

top=tkinter.Tk()

top.resizable(False, False)
# top.configure(background='white')
# set all font color as black
...
@th.no_grad()
def updateValue():
    go, state, action_mask, action_feats = mainGame.getCurrentState()
...
def updateProbNegLog(index):
    ...
    action_mask = th.from_numpy(action_mask).float()
    action_feats = [th.from_numpy(f).unsqueeze(0).float() for f in action_feats]
    index = th.from_numpy(np.array([index]))

    nlp = playerNetworks[go].neglogp(state, action_mask, action_feats, index).item()
    print(f"==>> nlp: {nlp}")
    prob = np.exp(-nlp)
    probNegLogValue.set(str(prob))
...
def sampleFromNetwork():
    ...
    go, state, actions, action_feats = mainGame.getCurrentState()
    state = th.from_numpy(state).float()
    actions = th.from_numpy(actions).float()
    action_feats = [th.from_numpy(f).unsqueeze(0).float() for f in action_feats]
    (a, v, nlp) = playerNetworks[go].step(state, actions, action_feats)
...
def loadNetworks():
    p1Name = p1Load.get()
    if p1Name != "":
        playerNetworks[1].load_state_dict(th.load(p1Name, map_location=th.device('cpu')))
    p2Name = p2Load.get()
    if p2Name != "":
        playerNetworks[2].load_state_dict(th.load(p2Name, map_location=th.device('cpu')))
    p3Name = p3Load.get()
    if p3Name != "":
        playerNetworks[3].load_state_dict(th.load(p3Name, map_location=th.device('cpu')))
    p4Name = p4Load.get()
    if p4Name != "":
        playerNetworks[4].load_state_dict(th.load(p4Name, map_location=th.device('cpu')))
    updateValue()
...
twidth = 30
p1Load = tkinter.Entry(top, width=twidth, background="white")
p2Load = tkinter.Entry(top, width=twidth, background="white")
p3Load = tkinter.Entry(top, width=twidth, background="white")
p4Load = tkinter.Entry(top, width=twidth, background="white")
diff = 30
sypoint = 70
sxpoint = 69
...
listBox = tkinter.Listbox(listFrame, height=10, background="white")
