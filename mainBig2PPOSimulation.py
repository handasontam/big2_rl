...
class big2PPOSimulation(object):
    ...
    def __init__(self, sess, *, inpDim = 412, nGames = 8, nSteps = 20, nMiniBatches = 4, nOptEpochs = 5, lam = 0.95, ent_coef = 0.01, vf_coef = 0.5, max_grad_norm = 0.5, minLearningRate = 0.000001, learningRate, clipRange, vfClipRange, saveEvery = 500, l2_coef = 1e-4):
        
        #network/model for training
        self.trainingNetwork = PPONetwork(sess, inpDim, 1695, "trainNet")
        self.trainingModel = PPOModel(sess, self.trainingNetwork, inpDim, 1695, ent_coef, vf_coef, max_grad_norm, l2_coef)
        ...
        self.rewardNormalization = 30.0 #divide rewards by this number (so reward ranges from -1.0 to 3.0)
    ...
    def run(self):
        ...
        for _ in range(self.nSteps):
            ...
            # print(f"==>> currAvailAcs: {type(currAvailAcs)}")
            # print(f"==>> currAvailAcs: {currAvailAcs}")
            # print(f"==>> currAvailAcs.shape: {currAvailAcs.shape}")
            # print(f"==>> currStates: {type(currStates)}")
            # print(f"==>> currStates: {currStates}")
            # print(f"==>> currStates.shape: {currStates.shape}")
            actions, values, neglogpacs = self.trainingNetwork.step(currStates, currAvailAcs)
            # print(f"==>> actions: {type(actions)}")
            # print(f"==>> actions: {actions}")
            # print(f"==>> actions.shape: {actions.shape}")
            # print(f"==>> values: {type(values)}")
            # print(f"==>> values: {values}")
            # print(f"==>> values.shape: {values.shape}")
            # print(f"==>> neglogpacs: {neglogpacs}")
            # print(f"==>> neglogpacs.shape: {neglogpacs.shape}")
            # print(f"==>> neglogpacs_softmax: {neglogpacs_softmax}")
            # print(f"==>> neglogpacs_softmax.shape: {neglogpacs_softmax.shape}")
        ...
        for k in range(4):
            ...
            for t in reversed(range(k, endLength, 4)):
                ...
                mb_advs[t] = lastgaelam = delta + self.lam * nextNonTerminal * lastgaelam
        ...
        for update in range(nUpdates):
            ...
            for _ in range(self.nOptEpochs):
                ...
                for start in range(0, batchSize, nTrainingBatch):
                    ...
                    mb_lossvals.append(self.trainingModel.train(lrnow, cliprangenow, vfcliprangenow, states[mb_inds], availAcs[mb_inds], returns[mb_inds], actions[mb_inds], values[mb_inds], neglogpacs[mb_inds]))
            ...
            for i, param in enumerate(newParams):
                print("param %d: average: %f, max: %f" % (i, np.average(np.abs(param)), np.max(np.abs(param))))
                # print(param.shape)
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        mainSim = big2PPOSimulation(sess, nGames=64, nSteps=80, learningRate = 0.00025, clipRange = 0.2, vfClipRange = 0.1, l2_coef=1e-5)
        start = time.time()
